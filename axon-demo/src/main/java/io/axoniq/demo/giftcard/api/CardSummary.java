...
public record CardSummary(String id, int initialValue, int remainingValue, Instant issued, Instant lastUpdated) {
    ...
    public CardSummary redeem(int amount, Instant redeemedAt) {
        return new CardSummary(this.id, this.initialValue, this.remainingValue - amount, this.issued, redeemedAt);
    }

    public CardSummary cancel(Instant cancelledAt) {
        return new CardSummary(this.id, this.initialValue, 0, this.issued, cancelledAt);
    }
}
