...
public class RefResolver extends PlayScriptBaseListener {
    ...
    private final List<FunctionCallContext> thisConstructorList = new LinkedList<>();
    private final List<FunctionCallContext> superConstructorList = new LinkedList<>();
    ...
    public void exitFunctionCall(FunctionCallContext ctx) {
        ...
        if (ctx.parent instanceof ExpressionContext exp) {
            if (exp.bop != null && exp.bop.getType() == PlayScriptParser.DOT) {
                Symbol symbol = at.symbolOfNode.get(exp.expression(0));
                if (symbol instanceof Variable && ((Variable) symbol).type instanceof Class theClass) {
    ...
    private boolean firstStatmentInFunction(FunctionDeclarationContext fdx, FunctionCallContext ctx) {
        return fdx.functionBody().block().blockStatements().blockStatement(0).statement() != null
